 ...
 * Verifies that dark mode can be toggled, persisted to localStorage,
 * and that UI elements update accordingly
 */
...
describe('Dark Mode Functionality', () => {
  ...
  /**
   * Helper function to simulate DOMContentLoaded event
   */
  ...
  describe('Mode Control Functions', () => {
    ...
    it('should return false from forceLightMode if already in light mode', () => {
      document.body.classList.remove('dark-mode');
      const wasDarkMode = forceLightMode();
      expect(wasDarkMode).toBe(false);
      expect(document.body.classList.contains('dark-mode')).toBe(false);
    });
    ...
    it('should keep light mode if restoreMode is called with false', () => {
      document.body.classList.remove('dark-mode');
      restoreMode(false);
      expect(document.body.classList.contains('dark-mode')).toBe(false);
    });
